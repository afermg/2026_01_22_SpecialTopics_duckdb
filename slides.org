...
** What we use in the (Python) shadows
...
** The MBTA (Massachusets Bay Transport Authority) publishes usage data
:PROPERTIES:
:BEAMER_opt: shrink=15
:END:
Let's explore it using DuckdDB

#+begin_src sql :session main :db test.duckdb :exports code :results none :eval no-export
  -- Remote calls
  INSTALL httpfs; 
  LOAD httpfs;
...
:PROPERTIES:
:header-args:duckdb: :session main :db test.duckdb :exports both
...
:BEAMER_opt: shrink=15
:END:
#+begin_src sql :session main :db test.duckdb :exports both :eval no-export
...
#+begin_src sql :session main :db test.duckdb :exports code :eval no-export :results none
...
#+begin_src sql :session main :db test.duckdb :exports both :results verbatim :wrap :eval no-export example 
  SET VARIABLE plates = 'https://github.com/jump-cellpainting/datasets/
  raw/refs/heads/main/metadata/plate.csv.gz';
...
** This can also be expressed in tree
#+begin_src sql :session main :eval no-export :exports code
...
#+begin_src sh :session duckdb_sh :results verbatim :wrap example :exports both :eval no-export
  DATA_DIR="/datastore/alan/gsk/aliby_output/ELN201687/H00DJKJread1BF48hrs_20230926_095825hive_profiles_tracked"

   tree --charset=ascii $DATA_DIR | sed -n "2,10p" 
#+end_src
...
** Hive partitioning
#+begin_src sql :eval no-export :session main :wrap example :results verbatim  
  SET VARIABLE hive = '/datastore/alan/gsk/aliby_output/ELN201687/H00DJKJread1BF48hrs_20230926_095825hive_profiles_tracked/*/*/*.parquet';
